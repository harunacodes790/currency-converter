...
function loadingFailed() {
  ...
  var t1 = document.createTextNode ("Ohh no! Curency Converter appears to be sleeping, please ");
...
function convertCurrencyCalculator(amount, base, target, output, unlockme) {
  ...
  if (window.usersLastConvInput != inputcompare) {
    ...
    var p = document.createElement("P");
    p.setAttribute("style", "color:blue; padding-top: 5px;");
    ...
    $.getJSON("https://api.coinbase.com/v2/exchange-rates?currency=" + base)
      ...
      var t = document.createTextNode("Seems you already asked for this conversion. Please change amount and try again.");
      var p = document.createElement("P");
      p.setAttribute("class", "input-currency-error-message");
      p.appendChild(t);
